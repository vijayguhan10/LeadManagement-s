...
export default function LoginPage({ setUserRole }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // If no role is selected, default to superadmin
      const actualRole = role === "" ? "superadmin" : role;
      ...
      if (actualRole === "superadmin") {
        ...
        database = "superadmin";
      } else if (actualRole === "telecaller") {
        database = "superadmin";
      }

      let response;
      if (actualRole === "telecaller") {
        response = await axios.post(
          `${process.env.REACT_APP_API_URL}/telecaller/login`,
          { email, password, role: actualRole, rememberMe },
          {
      ...
      <div className="w-full max-w-md bg-white p-8 rounded-2xl shadow-2xl">
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            disabled={isLoading}
            className={`w-full bg-blue-600 text-white py-3 px-4 rounded-lg font-medium shadow-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 ${
